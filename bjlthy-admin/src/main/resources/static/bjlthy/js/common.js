...
var refreshItem = function(){
    ...
    var target = $('.bjlthy_iframe[data-id="' + currentId + '"]', topWindow);
...
var closeItem = function(dataId){
	...
	if ($.common.isNotEmpty(dataId)) {
	    ...
	    // 移除相应tab对应的内容区
	    $('.mainContent .bjlthy_iframe[data-id="' + dataId + '"]', topWindow).remove();
	    return;
	}
	...
	if ($.common.isNotEmpty(panelUrl)) {
	    ...
	    $('.mainContent .bjlthy_iframe', topWindow).each(function() {
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="bjlthy_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" seamless></iframe>';
        if (isScrollToTop) {
            $('.mainContent', topWindow).find('iframe.bjlthy_iframe').hide().parents('.mainContent').append(str1);
        } else {
            $('.mainContent', topWindow).find('iframe.bjlthy_iframe').css({"visibility": "hidden", "position": "absolute"}).parents('.mainContent').append(str1);
        }
...
function activeWindow() {
    ...
    return $('.bjlthy_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
}
