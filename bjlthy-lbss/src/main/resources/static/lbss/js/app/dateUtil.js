 ...
 *      返回日期为: 2020-11-29
 * */
function currentWeekLastDay(dateStr) {
    let date = new Date(dateStr);
    let weekFirstDay = new Date(date - (date.getDay() - 1) * 86400000)
    let lastDay = new Date((weekFirstDay / 1000 + 6 * 86400) * 1000)
    ...
    return lastDay.getFullYear() + '-' + lastMonth + '-' + weekLastDays;
}
...
//加上时间
function addDay(dateStr, num) {

	var gdate = new Date(dateStr).getTime();//获取指定年月日
    gdate = gdate + 1000*60*60*24*num; //加减相差毫秒数
    var dateTime = new Date(gdate);//获取指定好毫秒数时间
...
function subtrDay(dayNumber, date) {
	var dateTime = new Date(date.year, date.month - 1, date.date, 0, 0, 0);
