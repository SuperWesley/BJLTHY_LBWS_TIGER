 ...
 * @version: V1.0
 * @author: 张宁
 * @description: 激光雷达状态 WebSocket
 * @date: 2020-12-05
 * @copyright: 北京龙田华远科技有限公司
 */
@Component
@ServerEndpoint("/ws/zy/lidarStateWebSocket/{belt_name}")
public class LidarStateWebSocket {
    /**
     * 在线数量
     */
    ...
    public static synchronized void addOnlineCount() {
        LidarStateWebSocket.onlineCount++;
    }
     ...
     */
    private static ConcurrentHashMap<String, LidarStateWebSocket> webSocketMap  = new ConcurrentHashMap<>();
    private static Map<String,String> stateMap = new HashMap<>();
    //日志工具类
    private static Logger log = LogbackUtil.getLogger("ZYBeltWebSocket", "ZYLidarStateWebSocket");
    ...
    public static synchronized  void sendMessageToAllClient(String message) {
        try {
            for (Map.Entry<String, LidarStateWebSocket> entry : webSocketMap.entrySet()) {
                LidarStateWebSocket webSocketServer = entry.getValue();
    ...
    public static synchronized  void sendMessageToClient(String belt_name,String message)  {
        try {
            synchronized(LidarStateWebSocket.class){
